...
```bash
docker run --rm -p 9080:8080 --name todo-app-backend ghcr.io/ws-4020/mobile-app-hands-on-backend:latest
```

H2に格納されているデータを永続化したい場合は、[Volume](https://docs.docker.com/storage/volumes/)を作成します。

```bash
docker run --rm -p 9080:8080 --name todo-app-backend -v todo-app-backend-volume:/usr/local/tomcat/h2 ghcr.io/ws-4020/mobile-app-hands-on-backend:latest
```
